...
export MY_APP_NAME=why-journal
createdb $MY_APP_NAME
createdb $MY_APP_NAME-test

```

* By default, running `npm test` will use `why-journal-test`, while
  regular development uses `why-journal`
...
process.env.GOOGLE_CALLBACK = '/auth/google/verify'
...
supported ways to deploy in Why Journal:
...
Why Journal comes with a `.travis.yml` configuration almost ready for
...
using Why Journal's handy deployment script:
